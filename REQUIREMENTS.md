...
> **実装反映（2026-03-05）**
> - FR-2/FR-3/FR-4 のP1相当として、認証・投稿・一覧・詳細・ページ単位 read/write 制御を実装済み。
> - admin運用は SQL 手順化済み（`mma-wiki-demo/supabase/admin_role_setup.sql`）。
...
1. **フェーズ1: 新歓デモ版（～2026年3月末）**
  ...
  - Supabase/Auth/Session出し分け/基本ロール管理までを先行実装
...
### 実装状況メモ（2026-03-01）
- P1のうち「認証・Session連動・部内ポータル導線・開発運用手順」は実装済み。
- 次の実装対象は `POST_TO_BLOG_MVP_PLAN.md` に定義した「Markdown投稿→ブログ一覧反映」。
